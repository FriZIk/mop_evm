...
void kuz_encrypt_block(kuz_key_t *key, void *blk)
{
    ...
    x.q[1] = ((uint64_t *) blk)[1];
            ...
            /*lab debug*/ printf("Шифрование \n");
            /*lab debug*/ printf("Исходный текст   ");
    ...
    for (i = 0; i < 9; i++) {
        ...
        x.q[1] ^= key->k[i].q[1];
            ...
            /*lab debug*/ printf("Раундовый ключ (%d)", i);
            /*lab debug*/ print_w128( (w128_t *) &key->k[i] );

            
     /*lab debug*/ printf("    Результат X ");
            /*lab debug*/ print_w128( (w128_t *) &x );

        for (j = 0; j < 16; j++)
            x.b[j] = kuz_pi[x.b[j]];

            /*lab debug*/ printf("    Результат PI");
            /*lab debug*/ print_w128( (w128_t *) &x );

        kuz_l(&x);

            /*lab debug*/ printf("    Результат L ");
            /*lab debug*/ print_w128( (w128_t *) &x );
...
void kuz_decrypt_block(kuz_key_t *key, void *blk)
{
    ...
    for (i = 8; i >= 0; i--) {
        ...
        kuz_l_inv(&x);
        /*lab*/ printf("    Результат обратного L ");
        /*lab*/ print_w128( (w128_t *) &x );
        
        for (j = 0; j < 16; j++)
            x.b[j] = kuz_pi_inv[x.b[j]];

        /*lab*/ printf("    Результат обратного PI");
        /*lab*/ print_w128( (w128_t *) &x );
        x.q[0] ^= key->k[i].q[0];   
        x.q[1] ^= key->k[i].q[1];
        
        /*lab*/ printf("Раундовый ключ (%d)", i);
        /*lab*/ print_w128( (w128_t *) &key->k[i] );

        /*lab*/ printf("    Результат X ");
        /*lab*/ print_w128( (w128_t *) &x );
    }
